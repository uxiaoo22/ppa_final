...
## Temporal Lags
Two temporal lag measures were added (We use Lag1 for following models):
...
```{r}
philly_tracts_2 <- st_read(
  "/Users/cathy/GitHub/ppa_final/copy_github2/data/tl_2020_42_tract/tl_2020_42_tract.shp"
)
...
# Then run your existing Moran's I code
...
corr_matrix <- cor(df[, vars_to_corr], use = "complete.obs")
...
```{r}
AIC(model1_ols, model2_lag_ols, model3_nb, model4_sar, model5_nb, model6_nb)
```
...
# --- Define quarters for cross-validation ---
...
model_names <- c("Model1_OLS", "Model2_TimeLag_OLS", "Model3_NB",
                 "Model5_ST_NB", "Model6_Final")
```
